...
@Autowired
DeviceMs(ApiConsumer apiConsumer, KafkaConsumer kafkaConsumer, KafkaProducer kafkaProducer)
{
	DeviceMs.apiConsumer = apiConsumer;
	DeviceMs.kafkaConsumer = kafkaConsumer;
	DeviceMs.kafkaProducer = kafkaProducer;
}
	public static void main(String[] args) throws InterruptedException {
		SpringApplication.run(DeviceMs.class, args);
		...
		//System.out.println(userInitializationDTO.getId());
...
/*
		...
		*/
		...
		DeviceMs.kafkaConsumer.addSubscriptionTopics(topics);
		while(true) {
			Thread.sleep(10000);
			DeviceMs.kafkaProducer.produceRecord("device.state.6985d5d5-a19d-4ffa-80ab-009ac08f88d1.atr1id1", "heheh1");
			DeviceMs.kafkaProducer.produceRecord("device.state.6985d5d5-a19d-4ffa-80ab-009ac08f88d1.atr1id", "heheh2");
