...
@Component
public class KafkaNewOnlineTopicRecordHandler implements KafkaChainHandler {
  //  @Autowiredc
   // ApiConsumer apiConsumer;
    ...
    public void handle(ToKafkaHandlerChainEntity entity, KafkaHandlerChain kafkaHandlerChain, int currentIndex) {
        ...
        try {
            //specialSystem.out.println("[KafkaNewOnlineTopicRecordHandler]");
            if (entity.getEntityType().equals(HandlerChainEntityType.TOPIC_NEW_RECORD)) {
                //specialSystem.out.println("[KafkaNewOnlineTopicRecordHandler]: new topic records "  );
                   ...
                   // ZoneOffset o = ZoneId.systemDefault().getRules().getOffset(Instant.now());
                    ...
                    try {
                        ...
                        { //specialSystem.out.println("it is ok");
                            ...
                            { //specialSystem.out.println("it is ok");
                                if( tokens.get(1).toLowerCase().equals("online"))
                                {
                                    //specialSystem.out.println("it is ok");
                                   ...
                                   // String attributeId = tokens.get(3);
                                    ...
                                    LocalDateTime date = LocalDateTime.ofInstant(Instant.ofEpochMilli(record.timestamp()), ZoneId.systemDefault());
                                    if(date.isBefore(LocalDateTime.now().minusSeconds(60)))
                                    {

                                        this.devicePool.handleOnlineUpdate(deviceId, false);
                                    ...
                                    }else
                                    {
                                        ...
                                        {
                                            Boolean online = objectMapper.treeToValue(jsonNode.get("online"), Boolean.class);
                                            this.devicePool.handleOnlineUpdate(deviceId, online);
