package com.liciot.humanms.kafka.consumer;
...
private Object lock = new Object();
                ...
                while (!closed.get()) {
                    ...
                    if (this.pool.get() && this.consumer.subscription().size() > 0) {
                        ...
                        for (ConsumerRecord<String, String> record : records) {
                             ...
                             System.out.println("[Consumed new message]: from topic" + record.topic() + " : " + record.value() +" "+LocalDateTime.ofInstant(Instant.ofEpochMilli(record.timestamp()), ZoneId.systemDefault()).toString());
                    ...
                    /*
                        // Handle new records
                        for (ConsumerRecord<String, String> record : records) {
                            //specialSystem.out.println("[Consumed new message]: from topic" + record.topic() + " : " + record.value());
                        }
    ...
    {
        //specialSystem.out.println("Synchronised");
    ...
    public  synchronized void addSubscriptionTopicsWithTime (Map<String,Long> subscriptionTopics)
    {
        ...
        {
            if(!this.subscriptionTopics.containsKey(topicEntry.getKey())) {
                //specialSystem.out.println("Added time");
                shouldSynchronize = true;
                this.subscriptionTopics.put(topicEntry.getKey(), 1);
                this.subscriptionTopicsState.put(topicEntry.getKey(), SubscriptionTopicState.JOINED_FOR_TIME);
                //specialSystem.out.println("[Will put] "+topicEntry.getKey()+" "+topicEntry.getValue());
    ...
    private class MyConsumerRebalanaceListener implements ConsumerRebalanceListener
    {
        ...
        public void onPartitionsRevoked(Collection<TopicPartition> partitions) {
            //specialSystem.out.println("onPartitionsRevoked - consumerName: , partitions:"+formatPartitions(partitions));
        }

        @Override
        public synchronized void onPartitionsAssigned(Collection<TopicPartition> partitions) {
            //specialSystem.out.println("onPartitionsAssigned - consumerName:, partitions: "+formatPartitions(partitions));
            ...
            for(TopicPartition topicPartition: partitions)
            {
                if(kafkaConsumer.getSubscriptionTopicsState().containsKey(topicPartition.topic()))
                {
                    ...
                    {
                        //specialSystem.out.println("If yes "+topicPartition.topic()+" "+kafkaConsumer.subscriptionTopicsTime.get(topicPartition.topic()));
                        newTopicPartitionsTime.put(topicPartition,kafkaConsumer.subscriptionTopicsTime.get(topicPartition.topic()));
                        //kafkaConsumer.getConsumer().seek(topicPartition,Long.max(kafkaConsumer.getConsumer().endOffsets(Arrays.asList(topicPartition)).get(topicPartition)-1,0L));
                        //kafkaConsumer.getSubscriptionTopicsState().put(topicPartition.topic(), SubscriptionTopicState.INITIALIZED);
                    }
                }
            ...
            {
                //specialSystem.out.println("[.onPartitionsAssigned] different sizes");

            }

                for(Map.Entry<TopicPartition,Long> topicPartitionEntry:topicPartitionsEndOffsets.entrySet())
                {
                    //specialSystem.out.println("[Topic partition seek] "+ topicPartitionEntry.getKey().topic()+" offset: "+topicPartitionEntry.getValue());
            ...
            //specialSystem.out.println("[Time]   "+newTopicPartitionsTime.size()+" "+newTopicPartitionsTime.keySet());
            ...
            {
                if(topicPartitionEntry.getValue()!=null) {
                    //specialSystem.out.println("[Topic partition time] " + topicPartitionEntry.getKey().topic() + " " + topicPartitionEntry.getValue().offset() + " " + LocalDateTime.ofInstant(Instant.ofEpochMilli(topicPartitionEntry.getValue().timestamp()), ZoneId.systemDefault()));
                    kafkaConsumer.getConsumer().seek(topicPartitionEntry.getKey(),Long.max( topicPartitionEntry.getValue().offset(),0L));

                }
                else
                {
                    //specialSystem.out.println("[Topic partition time null not seekeng ] " + topicPartitionEntry.getKey().topic()   );//" "+LocalDateTime.ofInstant(Instant.ofEpochMilli(topicPartitionEntry.getValue().timestamp()), ZoneId.systemDefault()));

                    //kafkaConsumer.getConsumer().seek(topicPartitionEntry.getKey(),Long.max( topicPartitionEntry.getValue().offset(),0L));

                    // //specialSystem.out.println("[Topic partition time] "+ topicPartitionEntry.getKey().topic()+" offset: "+topicPartitionEntry.getValue());
                }
                //kafkaConsumer.getConsumer().seek(topicPartitionEntry.getKey(),Long.max( topicPartitionEntry.getValue()-1,0L));
                //poate doar daac ma ingr3eruoe aii altfel ar trebui sa raman untd e trebuie offsetul pentru a este INITIALIZED
                //  kafkaConsumer.getSubscriptionTopicsState().put(topicPartitionEntry.getKey().topic(), SubscriptionTopicState.INITIALIZED);
